...
if pco_controller.get_status() is not 'configured':
    problems += 1
    print("Problem with configure()... status is not configured after configure()...")
...
print("pco_controller.get_configuration()... (after start)")
config_dict = pco_controller.get_configuration()
for key in config_dict:
    value = config_dict.get(key, None)
    if value is None:
        problems += 1
        print("Problem with %s from get_configuration() method while running..."%key)

# get_server_log while running
print("pco_controller.get_server_log()... (DUMMY LOG - after start)")
count = pco_controller.get_server_log().count('xbl-daq-32.psi.ch')  
if count != 10:
    problems += 1
    print("Problem with get_server_log() while running...")

print("pco_controller.get_server_uptime()... (DUMMY UPTIME - after start)")
if debug:
    uptime = pco_controller.get_server_uptime().split(" ")
    if uptime[0] != 'active' and uptime[-1] != 'CEST':
        problems += 1
        print("Problem with get_server_uptime() while running...")

print("pco_controller.get_statistics()... (after start)")
...
print("pco_controller.get_statistics_writer()... (after start)")
...
# gets status
print('pco_controller.status()... (after start)')
...
print("pco_controller.get_statistics_last_run()... (after start/stop)")
...
# gets status
print('pco_controller.status()... (after start/stop)')
if pco_controller.get_status() not in ['finished', 'stopping']:
    problems += 1
    print("Problem with get_status() after start/stop...")

# get_status_last_run
print('pco_controller.get_status_last_run()... (after start/stop)')
